...
; This section is fixed to always look for settings.ini in the script directory (A_ScriptDir)
; This ensures the MacroDir setting persists across script runs.
...
SetMacroDirectory:
    ...
    ; FIX: Always save settings.ini in the SCRIPT directory for easy retrieval on next run
    settingsPath := A_ScriptDir . "\settings.ini"
...
SaveMacro:
    ...
    ; Update main UI (This already worked because SaveMacro is called from the main GUI)
    GuiControl,, CurrentMacro, Current: %currentMacroName%
    
    ToolTip, Macro "%macroName%" saved and loaded.
    SetTimer, RemoveToolTip, -2000
return
...
; *** CORRECT SYNTAX FOR CROSS-GUI CONTROL IN AHK V1 ***
; The "1:" is now in the OPTIONS field (second parameter), making it work.
GuiControl, 1:, CurrentMacro, Current: %currentMacroName%
